...
#
# Description:
#     Controller for Managing DataSource records
#
# Copyright notice:
...
// List all DataSources available in the record
...
exports.orgsrc = (req, res) => {
    console.log("in");  
    let filter = {}
    if(req.params.org){
        var a=req.params.org
        a=a.substring(7)
        console.log(a);
        filter = { "dsname": { $regex: a, $options: 'i' } };
...
// Add new DataSource in the record
...
// Check whether the given DataSource exists on the record
...
// Get the selected DataSource from the list
...
// Update the given DataSource in the record
...
// list the database names in the Database server
...
// list the measurement names for the given database
...
// Delete the Selected DataSource from the record
...
// list the devices in slected DataSource based on devEUI/devID
exports.getDevList = async (req, res) => {
	...
	.then(async (data) => {
		if(data.length > 0) {
			...
			res.status(400).send({error: "Data source not found"});
		}
	})
	.catch(err => {
...
// Sub function of the "getDevList"
async function getDevices(dbdict, dtype){
    return new Promise(function(resolve, reject) {
        ...
        .then(json => {
            ...
            dres = json.results[0]
            if(dres.hasOwnProperty("series")){
                let infdevlist = dres.series[0].values;
                ...
                // console.log("No devices found under the data source")
                reject("No devices found under the data source")
            }
		    
        })
        .catch(err => {
            // console.log("Influx DB catch: ", err)
            reject("Error response from Influx database")
        });
    })
}